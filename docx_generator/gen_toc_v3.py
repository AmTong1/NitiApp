...
for i, page in enumerate(pdf.pages):
    ...
    for line in text.split('\n'):
        ...
        if line in ['บทคัดย่อ','ABSTRACT','กิตติกรรมประกาศ','บรรณานุกรม']:
            raw_pg.setdefault(line, pn)
        if re.match(r'^บทที่\s*\d+$', line):
            raw_pg.setdefault(line, pn)
        m = re.match(r'^(\d+\.\d+)\s', line)
        if m: raw_pg.setdefault(m.group(1), pn)
        m = re.match(r'(รูปที่?\s*\d+\.\d+)', line)
        if m: raw_pg.setdefault(m.group(1).strip(), pn)
        m = re.match(r'(ตารางที่?\s*[\d.]+)', line)
        if m: raw_pg.setdefault(m.group(1).strip().rstrip(':'), pn)
pdf.close()

# Chapter 1 starts at PDF page 10 => document page 1
OFFSET = 9
...
# Front matter with Thai letters
...
# Ch1
entry(newdoc, "บทที่ 1 บทนำ", 1, P("บทที่ 1"))
for s,n in [("1.1","ที่มาและความสำคัญของปัญหา"),("1.2","วัตถุประสงค์ของโครงงาน"),
            ("1.3","ขอบเขตของโครงงาน"),("1.4","แผนดำเนินการและระยะเวลา"),
            ("1.5","ประโยชน์ที่คาดว่าจะได้รับ"),("1.6","งบประมาณที่ใช้ในการจัดทำโครงงาน")]:
    entry(newdoc, f"{s} {n}", 2, P(s))
blank(newdoc)

# Ch2
entry(newdoc, "บทที่ 2 ทฤษฎีและงานวิจัยที่เกี่ยวข้อง", 1, P("บทที่ 2"))
...
# Ch3
entry(newdoc, "บทที่ 3 วิธีการดำเนินงาน", 1, P("บทที่ 3"))
...
# Ch4
entry(newdoc, "บทที่ 4 ผลการดำเนินงาน", 1, P("บทที่ 4"))
for s,n in [("4.1","ผลของการพัฒนาระบบ"),("4.2","ผลการทดสอบความถูกต้องของลิงก์และการนำทาง"),
            ("4.3","ผลการทดสอบการยอมรับของผู้ใช้งาน (UAT)"),("4.4","ผลการประเมินประสิทธิภาพและความพึงพอใจ")]:
    entry(newdoc, f"{s} {n}", 2, P(s))
blank(newdoc)

# Ch5
entry(newdoc, "บทที่ 5 สรุปผล อภิปรายผล และข้อเสนอแนะ", 1, P("บทที่ 5"))
for s,n in [("5.1","สรุปผลการดำเนินงาน"),("5.2","อภิปรายผล"),
            ("5.3","ปัญหาและอุปสรรค"),("5.4","ข้อเสนอแนะเพื่อการพัฒนาต่อยอด")]:
    entry(newdoc, f"{s} {n}", 2, P(s))
blank(newdoc)
entry(newdoc, "บรรณานุกรม", 0, P("บรรณานุกรม"))
...
out = r"C:\Users\tanut\Downloads\สารบัญ_NitiSmart_V3.docx"
newdoc.save(out)
print(f"Saved: {out}")
