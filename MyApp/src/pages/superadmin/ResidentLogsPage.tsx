...
type ResidentLog = {
  ...
  created_at: string;
  current_resident_status?: string | null;
};
...
const ResidentLogsPage: React.FC<ResidentLogsPageProps> = ({ onBack }) => {
  ...
  const handleRestore = async (logItem: ResidentLog) => {
    const residentId = Number(logItem.resident_id);
    if (!Number.isFinite(residentId) || residentId <= 0) return;
    Alert.alert('ยืนยันการกู้คืน', 'ต้องการกู้คืนข้อมูลลูกบ้านใช่หรือไม่?', [
      { text: 'ยกเลิก', style: 'cancel' },
      { text: 'กู้คืน', style: 'default', onPress: async () => {
          try {
            const token = await AsyncStorage.getItem('token');
            const res = await fetch(`${getBaseUrl()}/residents/${residentId}/restore`, {
              method: 'POST',
              headers: { Authorization: `Bearer ${token}` }
            });
            const json = await res.json();
            if (!res.ok || !json?.ok) throw new Error(json?.error || 'กู้คืนไม่สำเร็จ');
            setRestoredIds(prev => new Set(prev).add(residentId));
            Alert.alert('สำเร็จ', 'กู้คืนข้อมูลลูกบ้านเรียบร้อยแล้ว');
            fetchLogs();
          } catch (e: any) {
            if (e.message === 'EXPIRED_30_DAYS') {
               Alert.alert('ผิดพลาด', 'ไม่สามารถกู้คืนได้เนื่องจากเลยกำหนดเวลา 30 วันแล้ว');
            } else {
  ...
  const renderLog = ({ item }: { item: ResidentLog }) => {
    ...
    const residentId = Number(item.resident_id);
    const isRestored = Number.isFinite(residentId) && residentId > 0
      ? restoredIds.has(residentId) || item.current_resident_status === 'active'
      : false;
      ...
      >
        ...
        {isExpanded && item.changes && (
          <View style={styles.expandedSection}>
            ...
            {item.action === 'delete' && !isRestored && (
              <TouchableOpacity 
                style={styles.restoreBtn}
                onPress={() => handleRestore(item)}
              >
                <Ionicons name="refresh-circle" size={20} color="#fff" />
                <Text style={styles.restoreBtnText}>กู้คืนข้อมูล (ภายใน 30 วัน)</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
