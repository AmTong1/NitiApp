...
const PaymentHistory: React.FC<Props> = ({ house: propHouse, houseNumber, isAdmin, onGoQr }) => {
  ...
  const house = propHouse ?? fromHouseNumber ?? paramHouse;
  ...
  const renderInstallmentSection = (item: Payment) => {
    ...
    if (list.length > 0) {
      ...
      if (isAdminView) {
        const totalMonths = list.reduce((sum, r) => sum + (Number(r.months_span) || 1), 0);
        const totalAmount = list.reduce((sum, r) => sum + Number(r.amount || 0), 0);
        const todayForAdmin = new Date();
        const todayKeyForAdmin = new Date(todayForAdmin.getFullYear(), todayForAdmin.getMonth(), todayForAdmin.getDate()).getTime();
        return (
          <View style={styles.mt10}>
            <Text style={styles.scheduleHeader}>
              {t('phTotal')} {fmt(totalAmount)} {t('phBaht')} · {list.length} {t('phInstallments')} ({totalMonths} {t('phMonthUnit')}) · {t('phStartFrom')} {fmtDate(start)}
            </Text>
            <View style={styles.scheduleWrap}>
              {list.map((row) => {
                ...
                const dueTime = parseMySqlDateTime(row.due_date).getTime();
                const isOldCycle = dueTime < todayKeyForAdmin;
                const monthsSpan = Number(row.months_span) || 1;
                const cycleLabel = isOldCycle ? t('phCycleOld') : t('phCycleNew');
                const monthsLabel = t('phCycleMonths', { months: String(monthsSpan) });
                return (
                  <TouchableOpacity
                    key={row.id}
                    activeOpacity={0.8}
                  ...
                  >
                    <View>
                      ...
                      <Text style={styles.scheduleMeta}>
                        {Number(row.discount_amount || 0) > 0 && row.original_amount ? (
                          <>
                            <Text style={styles.originalAmountText}>{fmt(Number(row.original_amount))} </Text>
                            <Text style={styles.discountedAmountText}>{fmt(row.amount)} {t('phBaht')}</Text>
                            <Text style={styles.discountBadgeText}> (-{fmt(Number(row.discount_amount))})</Text>
                          </>
                        ) : (
                          <>{fmt(row.amount)} {t('phBaht')} ({cycleLabel} · {monthsLabel})</>
                        )}
                      </Text>
                    </View>
                  </TouchableOpacity>
      ...
      const statusForUser = (r: PaymentInstallment) => getInstallmentStatus(r);
      const paidListForUser = list.filter(r => statusForUser(r) === 'paid');
      const paidCountForUser = paidListForUser.length;
      const paidAmountForUser = paidListForUser.reduce((sum, r) => sum + Number(r.amount || 0), 0);
      
      const renderUserSummary = () => {
        if (paidCountForUser === 0) return null;
        return (
          <View style={styles.userSummaryBox}>
            <View style={styles.userSummaryIcon}>
              <Ionicons name="wallet-outline" size={24} color="#0D9488" />
            </View>
            <View style={styles.userSummaryContent}>
              <Text style={styles.userSummaryText}>ชำระแล้ว {paidCountForUser} งวด</Text>
              <Text style={styles.userSummarySub}>เป็นเงิน {fmt(paidAmountForUser)} บาท</Text>
            </View>
          </View>
        );
      };

      const normalizeProofUrl = (proof?: string | null) => {
      ...
      const openReceipt = (row: PaymentInstallment) => {
        const url = normalizeProofUrl(row.proof_image);
        if (!url) {
          showAlert('ไม่พบใบเสร็จ', 'งวดนี้ยังไม่มีรูปใบเสร็จ');
          return;
        }
        setViewImageUri(url);
      };

      const renderPaidHistory = (withTopMargin: boolean) => {
        if (paidListForUser.length === 0) return null;
        return (
          <>
            <Text style={[styles.listHeader, withTopMargin && styles.mt12]}>ประวัติการชำระเงิน (Paid)</Text>
            {paidListForUser.map((row) => {
              return (
                <TouchableOpacity key={row.id} style={styles.userInstRow} activeOpacity={0.8}
                  onPress={() => openReceipt(row)}>
                  <View style={styles.userCol}>
                    <Text style={styles.userLabel}>{t('phDateLabel')}</Text>
                  ...
                  <View style={styles.userCol}>
                    ...
                    <Text style={styles.userValue}>
                      {fmt(row.amount)} {t('phBaht')}{'\n'}
                      <Text style={styles.userMeta}>({row.months_span || 1} {t('phMonthUnit')})</Text>
                    </Text>
                  </View>
                  <View style={styles.userStatusCol}>
                    <Text style={[styles.userStatus, styles.userStatusPaid]}>{t('phPaidComplete')}</Text>
                  </View>
                </TouchableOpacity>
              );
            })}
      ...
      const todayForUser = new Date();
      const todayKeyForUser = new Date(todayForUser.getFullYear(), todayForUser.getMonth(), todayForUser.getDate()).getTime();
      const timeForUser = (r: PaymentInstallment) => parseMySqlDateTime(r.due_date).getTime();
      const notPaidForUser = (r: PaymentInstallment) => {
        const st = statusForUser(r);
        return st !== 'paid' && st !== 'waiting_approval';
      };
      
      const isUpcomingVisibleForUser = (r: PaymentInstallment) => {
        ...
        const diffDays = (timeForUser(r) - todayKeyForUser) / (1000 * 60 * 60 * 24);
        return diffDays <= thresholdDays;
      };

      const upcomingList = list
        .filter((r) => notPaidForUser(r) && timeForUser(r) >= todayKeyForUser && !overdueList.some(o => o.id === r.id) && isUpcomingVisibleForUser(r))
        .sort((a, b) => timeForUser(a) - timeForUser(b))
        .slice(0, 1);

      if (overdueList.length === 0 && upcomingList.length === 0) {
        const anyUnpaid = list.some(r => r.status !== 'paid');
        if (!anyUnpaid) {
          return (
            <View style={styles.mt10}>
              {renderUserSummary()}
        ...
        const latestSt = getInstallmentStatus(latest);
        const latestStatusLabel = latestSt === 'paid' ? t('phPaidComplete') :
          latestSt === 'waiting_approval' ? t('phWaitingApproval') :
          latestSt === 'overdue' ? t('payStatusOverdue') : t('payStatusPending');
        const latestStatusStyle = latestSt === 'paid' ? styles.userStatusPaid :
          latestSt === 'overdue' ? styles.userStatusOverdue : styles.userStatusPending;
        return (
          <View style={styles.mt10}>
            {renderUserSummary()}
            <Text style={styles.listHeader}>{t('phLatestInstallment')}</Text>
            <TouchableOpacity style={styles.userInstRow} activeOpacity={0.8}
              ...
              <View style={styles.userCol}>
                <Text style={styles.userLabel}>{t('phAmountLabel')}</Text>
                <Text style={styles.userValue}>
                  {fmt(latest.amount)} {t('phBaht')}{'\n'}
                  <Text style={styles.userMeta}>({latest.months_span || 1} {t('phMonthUnit')})</Text>
                </Text>
              </View>
              <View style={styles.userStatusCol}>
                <Text style={[styles.userStatus, latestStatusStyle]}>{latestStatusLabel}</Text>
              </View>
            </TouchableOpacity>
            {renderPaidHistory(true)}
          </View>
      ...
      return (
        <View style={styles.mt10}>
          ...
          {upcomingList.length > 0 && (
            <>
              <Text style={[styles.listHeader, overdueList.length > 0 && styles.mt12]}>{t('phUpcomingInstallments')}</Text>
              {upcomingList.map((row) => {
                const rowStatus = getInstallmentStatus(row);
                const rowStatusLabel = rowStatus === 'paid' ? t('phPaidComplete') :
                  rowStatus === 'overdue' ? t('payStatusOverdue') : t('payStatusPending');
                const rowStatusStyle = rowStatus === 'paid' ? styles.userStatusPaid :
                  rowStatus === 'overdue' ? styles.userStatusOverdue : styles.userStatusPending;
                return (
                  <TouchableOpacity key={row.id} style={styles.userInstRow} activeOpacity={0.8}
                    onPress={() => goQr(row, item.house_number)}>
                    <View style={styles.userCol}>
                    ...
                    <View style={styles.userStatusCol}>
                      <Text style={[styles.userStatus, rowStatusStyle]}>{rowStatusLabel}</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
          ...
          {}
          {renderPaidHistory(overdueList.length > 0 || upcomingList.length > 0)}
        </View>
      );
    }
    ...
    if (isAdminView) {
      const fallbackTotalMonths = count * m;
      const fallbackTotalAmount = (perInstallment || 0) * count;
      return (
        <View style={styles.mt10}>
          <Text style={styles.scheduleHeader}>
            {t('phTotal')} {fmt(fallbackTotalAmount)} {t('phBaht')} · {count} {t('phInstallments')} ({fallbackTotalMonths} {t('phMonthUnit')}) · {t('phStartFrom')} {fmtDate(start)}
          </Text>
          <View style={styles.scheduleWrap}>
...
const styles = StyleSheet.create({
  ...
  scheduleMeta: { fontWeight: '600', fontSize: 10, color: '#6B7280', marginTop: 2 },
  originalAmountText: { textDecorationLine: 'line-through', color: '#9CA3AF' },
  discountedAmountText: { color: '#10B981', fontWeight: '700' },
  discountBadgeText: { color: '#F59E0B', fontWeight: '600' },
  ...
  userMeta: { fontSize: 11, fontWeight: '600', color: '#64748B' },
...
const pickNextInstallment = (list: PaymentInstallment[]) => {
  ...
  const notPaid = (r: PaymentInstallment) => {
    const st = getInstallmentStatus(r);
    return st !== 'paid' && st !== 'waiting_approval';
  };
  ...
  const overdue = list
    .filter((r) => {
      ...
      if (getInstallmentStatus(r) === 'overdue') return true;
...
const getInstallmentStatus = (r: PaymentInstallment): 'pending' | 'paid' | 'overdue' | 'waiting_approval' => {
  ...
  if (rawStatus === 'waiting_approval') return 'waiting_approval';
  if (rawStatus === 'paid' || rawStatus === 'success' || rawStatus === 'completed') return 'paid';
  if (r.paid_at && rawStatus !== 'waiting_approval') return 'paid';
