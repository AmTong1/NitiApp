...
const ResidentForm: React.FC<{
  ...
  const [payMonths, setPayMonths] = useState(initial?.pay_months != null ? String(initial.pay_months) : '');
              ...
              >
                ...
                <TextInput
                  ...
                  onChangeText={(t) => {
                    ...
                    setHouse(filtered);
                    if (filtered.trim()) setHouseError(false);
                  }}
                  placeholder="เช่น 101 หรือ 132/1"
...
const UserManage: React.FC<Props> = ({ darkMode }) => {
  ...
  const [meRole, setMeRole] = useState<string>('admin');
  const [confirmDeleteCheck, setConfirmDeleteCheck] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchMe = async () => {
      try {
        ...
        if (res.ok) {
          const json = await res.json();
          if (json.user?.role) setMeRole(json.user.role);
        }
      } catch (e) {}
    };
    fetchMe();
  ...
  const executeDelete = async (item: Resident) => {
    try {
      ...
      if (json.status === 'pending_approval') {
        setAlertModal2({ title: 'ส่งคำขอแล้ว', message: 'ระบบได้ส่งคำขอลบลูกบ้านไปยัง Super Admin แล้ว รอการอนุมัติ' });
      } else {
        setAlertModal2({ title: 'สำเร็จ', message: 'ลบลูกบ้านเรียบร้อยแล้ว' });
      }
      await fetchList();
          ...
          <View style={styles.modalCenter}>
            <View style={[styles.alertCard, themed.alertCard]}>
              ...
              {meRole === 'superadmin' && (
                <TouchableOpacity
                  activeOpacity={0.8}
                  style={[styles.deleteCheckRow, darkMode ? styles.deleteCheckRowDark : styles.deleteCheckRowLight]}
                  onPress={() => setConfirmDeleteCheck(!confirmDeleteCheck)}
                >
                  <Ionicons name={confirmDeleteCheck ? 'checkbox' : 'square-outline'} size={24} color={confirmDeleteCheck ? '#EF5350' : '#888'} />
                  <Text style={[styles.deleteCheckText, darkMode ? styles.deleteCheckTextDark : styles.deleteCheckTextLight]}>ยืนยันการลบลูกบ้าน (กู้คืนได้ภายใน 30 วัน)</Text>
                </TouchableOpacity>
              )}
              ...
              <View style={styles.confirmActions}>
                <TouchableOpacity
                  style={[styles.confirmBtn, styles.confirmBtnCancel]}
                  onPress={() => { setDeleteConfirm(null); setConfirmDeleteCheck(false); }}
                >
                  <Text style={styles.confirmBtnCancelText}>ยกเลิก</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.confirmBtn, styles.confirmBtnDanger, (meRole === 'superadmin' && !confirmDeleteCheck) && styles.confirmBtnDisabled]}
                  disabled={meRole === 'superadmin' && !confirmDeleteCheck}
                  onPress={() => { const item = deleteConfirm; setDeleteConfirm(null); setConfirmDeleteCheck(false); if (item) executeDelete(item); }}
                >
                  <View style={styles.rowCenterGap6}>
                    <Ionicons name="trash-outline" size={16} color="#fff" />
                    <Text style={styles.confirmBtnSaveText}>{meRole === 'superadmin' ? 'ลบ' : 'ขออนุมัติลบ'}</Text>
                  </View>
                </TouchableOpacity>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  deleteCheckRow: { flexDirection: 'row' as const, alignItems: 'center' as const, marginVertical: 10, padding: 10, borderRadius: 8 },
  deleteCheckRowDark: { backgroundColor: '#333' },
  deleteCheckRowLight: { backgroundColor: '#F3F4F6' },
  deleteCheckText: { marginLeft: 8, fontSize: 14 },
  deleteCheckTextDark: { color: '#fff' },
  deleteCheckTextLight: { color: '#333' },
}); 
