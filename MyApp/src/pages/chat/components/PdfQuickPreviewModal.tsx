...
function PdfQuickPreviewModalImpl({
  ...
  const waitForCacheWarmup = useCallback(async (localPath: string, tempPath: string) => {
    // If prefetch is already writing the same file, wait briefly for it to finish.
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (partExists) {
        ...
        // Stale temp file: do not block preview too long.
        if (stagnantPartCount >= 15) return false;
      }

      // Give prefetch a short grace period to create the .part file.
  ...
  useEffect(() => {
    ...
    // Some PDFs render but don't always trigger onLoadComplete reliably on all devices.
    // Auto-clear the loading overlay after a short grace period once local file is ready.
