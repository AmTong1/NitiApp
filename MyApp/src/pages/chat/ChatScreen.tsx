...
const FIXED_UPLOAD_ORIGIN = getBaseUrl();
...
const ChatScreen: React.FC<ChatScreenProps> = ({ darkMode = false, initialRoom, onBack }) => {
    ...
    try {
      ...
      xhr.upload.onprogress = (e: any) => {
        if (e && e.lengthComputable) {
          ...
          const bucket = rawP >= 0.995 ? 100 : Math.floor(rawP * 20);
          const now = Date.now();
    ...
    return (
      <View>
        ...
        <View style={[
          ...
          { marginBottom: attachToPrev ? 4 : 12 },
        ]}>
          {!mine ? ((showAvatar)
  ...
  const renderGridRow = useCallback((unit: Extract<RenderUnit, { kind: 'grid' }>) => {
    ...
    const CONTAINER_MAX = Math.floor((SCREEN_W - 24) * 0.78);
    const GAP = 6;
    const count = unit.items.length;
    const COLS = 2;
    const MAX_ROWS = 2;
    const maxTiles = COLS * MAX_ROWS;
    const overlayMode = count > maxTiles;
